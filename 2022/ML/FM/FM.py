...
def FM_SGD(X,
    ...
    # 初始化隐向量矩阵V=(n, k)~N(0, 1)，其中Vj是第j维特征的隐向量
...
def draw_research(all_FM_params, X_train, y_train, X_test, y_test):
    ...
    for w0, W, V in all_FM_params:
        ...
        for i in range(X_train.shape[0]):
            total_loss += logit(y=y_train[i],
                                y_hat=FM(Xi=X_train[i], w0=w0, W=W, V=V))
        all_total_loss.append(total_loss / X_train.shape[0])
        all_total_accuracy.append(
            accuracy_score(y_test, FM_predict(X=X_test, w0=w0, W=W, V=V)))
    plt.plot(np.arange(len(all_FM_params)),
             all_total_loss,
             color='#FF4040',
             label='训练集的损失值')
    plt.plot(np.arange(len(all_FM_params)),
...
def draw_weights(w0, W, V):
    weights_list = [w0] + list(W)
    W1 = np.dot(V, V.T)
    n = np.shape(W1)[0]
    for i in range(n - 1):
        for j in range(i + 1, n):
            weights_list.append(W1[i][j])
    length = [i for i in range(len(weights_list))]
    plt.stem(length, weights_list)
    plt.show()
...
if __name__ == '__main__':
    ...
    '''*****************FM预测模型*****************'''
    # SGD更新FM模型的参数列表：[w0, W, V]
    all_FM_params = FM_SGD(X=X_train,
                           y=y_train,
                           k=2,
                           alpha=0.01,
                           iter=45,
